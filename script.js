...
async function runTradeScanner() {
    ...
    try {
        ...
        for (const item of results.slice(0, 20)) {
            ...
            row.innerHTML = `
        <td>${item.name}</td>
        <td class="num">${formatNumber(item.bestBuy)}</td>
        <td class="num">${formatNumber(item.bestSell)}</td>
        <td class="num">${formatNumber(item.margin, 1)}%</td>
...
async function runCompressedOre() {
    ...
    try {
        ...
        for (const ore of results) {
            ...
            row.innerHTML = `
        <td>${ore.name}</td>
        <td class="num">${formatNumber(ore.bestBuy)}</td>
        <td class="num">${formatNumber(ore.bestSell)}</td>
        <td class="num">${formatNumber(ore.margin, 1)}%</td>
        <td class="num">${formatVolume(ore.volume)}</td>
      `;
            tbody.appendChild(row);
        }
    } catch (error) {
...
function buildProfitCell(value) {
    let className = "num";
    if (value > 0) className += " profit";
    if (value < 0) className += " loss";
...
async function runRefineCalculator() {
    ...
    try {
        ...
        for (const rowData of rows) {
            ...
            row.innerHTML = `
        <td>${rowData.name}</td>
        <td class="num">${formatNumber(rowData.orePrice)}</td>
        <td class="num">${formatNumber(rowData.mineralValue)}</td>
        ${buildProfitCell(rowData.profit)}
      `;
            tbody.appendChild(row);
        }
    } catch (error) {
